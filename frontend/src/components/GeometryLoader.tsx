...
// Progressive loading messages that cycle randomly after 3 seconds
...
interface GeometryLoaderProps {
  initialMessage?: string;
}

const GeometryLoader: React.FC<GeometryLoaderProps> = ({ initialMessage = "Preparing your workspace..." }) => {
  const [currentMessage, setCurrentMessage] = useState(initialMessage);
  const [showProgressive, setShowProgressive] = useState(false);

  useEffect(() => {
    // Start showing progressive messages after 3 seconds
  ...
  useEffect(() => {
    ...
    // Get random messages from the array (excluding initial message if it's similar)
    const getRandomMessage = () => {
      const availableMessages = LOADING_MESSAGES.filter(
        msg => msg !== initialMessage && msg !== currentMessage
      );
      const randomIndex = Math.floor(Math.random() * availableMessages.length);
      return availableMessages[randomIndex];
    };

    // Cycle through messages every 2.5 seconds with random selection
    const interval = setInterval(() => {
      setCurrentMessage(getRandomMessage());
    }, 2500);

    return () => clearInterval(interval);
  }, [showProgressive, initialMessage, currentMessage]);
